...
function HomeEvents({ upcoming, completed, loading }: { upcoming: HomeEvent[]; completed: HomeEvent[]; loading: boolean }) {
  return <section className="home-events" aria-labelledby="home-events-title"><div className="home-events-heading"><div><p className="eyebrow"><span>AGENDA</span> DRAFTS Y TORNEOS</p><h2 id="home-events-title">EVENTOS<br />EN KEKE.</h2></div><p>Consulta lo que viene y revive los resultados de cada evento competitivo.</p><a href="/eventos">VER TODOS LOS EVENTOS <span>→</span></a></div>{loading ? <p className="home-events-empty">CARGANDO EVENTOS…</p> : <div className="home-events-grid"><HomeEventGroup title="POR REALIZAR" events={upcoming.slice(0, 2)} empty="Todavía no hay eventos próximos publicados." /><HomeEventGroup title="REALIZADOS" events={completed.slice(0, 2)} empty="Aún no hay eventos finalizados." completed /></div>}</section>;
}

function HomeEventGroup({ title, events, empty, completed = false }: { title: string; events: HomeEvent[]; empty: string; completed?: boolean }) {
  return <div className="home-event-group"><header><div><span>{completed ? "ARCHIVO" : "CALENDARIO"}</span><strong>{title}</strong></div><b>{String(events.length).padStart(2, "0")}</b></header>{events.length ? events.map((event) => <a className="home-event-row" href={`/eventos/${encodeURIComponent(event.slug)}`} key={event.id}>{event.cover_image ? <img src={event.cover_image} alt="" /> : <span className="home-event-letter">{event.metadata?.type === "draft" ? "D" : "T"}</span>}<div><small>{event.metadata?.type === "draft" ? "DRAFT" : "TORNEO"} · {completed ? "FINALIZADO" : "PUBLICADO"}</small><strong>{event.title}</strong><p>{completed ? eventWinner(event) : event.metadata?.date ?? "Fecha por anunciar"}</p></div><b aria-hidden="true">↗</b></a>) : <div className="home-events-empty"><span>—</span><p>{empty}</p><a href="/eventos">ABRIR CALENDARIO</a></div>}</div>;
}
...
function FeaturedEvent({ event, completed, loading }: { event?: HomeEvent; completed: boolean; loading: boolean }) {
  ...
  return <article className={`tournament-card${event?.cover_image ? " has-event-cover" : ""}`} style={event?.cover_image ? { backgroundImage: `url(${event.cover_image})` } : undefined}><div className="event-cover-shade" aria-hidden="true" /><div className="card-kicker"><span>{completed ? "ÚLTIMO EVENTO" : "PRÓXIMO EVENTO"}</span><time>{event ? completed ? "FINALIZADO" : event.metadata?.date ?? "POR ANUNCIAR" : "SIN PUBLICAR"}</time></div><div className="trophy-mark" aria-hidden="true">{event?.metadata?.type === "draft" ? "DRAFT" : "EVENT"}</div><div className="tournament-copy"><p>{event?.metadata?.type === "draft" ? "DRAFT OFICIAL" : "TORNEO OFICIAL"}</p><h2>{title}</h2><div className="prize"><small>{completed ? "EQUIPO GANADOR" : "FECHA Y FORMATO"}</small><strong>{event ? completed ? eventWinner(event) : `${event.metadata?.date ?? "Por anunciar"}${event.metadata?.format ? ` · ${event.metadata.format}` : ""}` : "PUBLICA UN EVENTO DESDE EL PANEL"}</strong></div><a className="tournament-action" href={event ? `/eventos/${encodeURIComponent(event.slug)}` : "/eventos"}>{event ? "VER EVENTO" : "VER CALENDARIO"} <span>→</span></a></div></article>;
}
