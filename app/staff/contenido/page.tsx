        ...
        <div className="staff-main">
          ...
          {allowed && <>
            ...
            <form className="content-form draft-publication-form" onSubmit={handleSubmit}>
              ...
              {publication === "history" && <section className="publication-form-section history-publication-section"><header><span>04</span><div><strong>RESULTADO FINAL</strong><small>Equipo ganador e integrantes vinculados a cuentas verificadas</small></div></header><div className="form-row winner-result-fields"><div className="form-field"><label htmlFor="winner-team">Nombre del equipo ganador</label><RichTextEditor id="winner-team" required rows={2} value={winnerTeam} onChange={setWinnerTeam} mentions={winningCaptain ? [winningCaptain] : []} placeholder="Team de @capitán" />{winningCaptain && <small className="field-help">Pulsa “@ {winningCaptain.minecraft_username}” para vincular al capitán en el nombre del equipo.</small>}</div><label>Equipo finalista<input value={runnerUp} onChange={(event) => setRunnerUp(event.target.value)} placeholder="Subcampeón (opcional)" /></label></div><VerifiedPlayerPicker title="Integrantes ganadores" description="Busca y añade a todos los miembros del equipo ganador" selected={winners} onChange={setWinners} /><VerifiedPlayerPicker title="Menciones honoríficas" description="Opcional: selecciona uno o varios jugadores destacados" selected={honorableMentions} onChange={setHonorableMentions} />{honorableMentions.length > 0 && <label>Motivo de la mención<input value={honorableReason} onChange={(event) => setHonorableReason(event.target.value)} placeholder="Defensa destacada, MVP, mejor capitán…" /></label>}</section>}
